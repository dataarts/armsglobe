...
const vec3_x_axis = new THREE.Vector3( 1, 0, 0 ); // eslint-disable-line camelcase
const vec3_y_axis = new THREE.Vector3( 0, 1, 0 ); // eslint-disable-line camelcase
const vec3_z_axis = new THREE.Vector3( 0, 0, 1 ); // eslint-disable-line camelcase
...
class ParticleMesh {
  constructor( vizMesh ) {
    ...
    const explosionGeo = new THREE.SphereGeometry( 5, 32, 32 );
    ...
    this.explosionSphere.update = function explosionSphereUpdate( traceLine ) {
      ...
      for (let index = 0; index < traceLine.geometry.vertices.length; index++) {
        ...
        const vertex = traceLine.geometry.vertices[ index ];
    ...
    this.pSystem.update = function pSystemUpdate( traceLine ) {
  ...
  runExplosion() {
    const path = this.particlesGeo.vertices[0].path;
    ...
    switch (choice) { // eslint-disable-line default-case
    case 0:
      this.explosionSphere.rotationAxis = vec3_x_axis; // eslint-disable-line camelcase
      break;
    case 1:
      this.explosionSphere.rotationAxis = vec3_y_axis; // eslint-disable-line camelcase
      break;
    case 2:
      this.explosionSphere.rotationAxis = vec3_z_axis; // eslint-disable-line camelcase
      break;
    }
...
export function buildDataVizGeometries( linearData, countryData ) {
  const loadLayer = document.getElementById( 'loading' );

  linearData.forEach( (set) => {
    const exporterName = set.src.toUpperCase();
    const importerName = set.dest.toUpperCase();

    const exporter = countryData[exporterName];
    const importer = countryData[importerName];
...
export function initParticleViz( linearData ) {
  ...
  _getMeshFromPool( ( meshObj ) => {
    // get our colour
    const color = _getColourFromTypeStr( linearData.colour );

    // merge it all together
    meshObj.linesGeo.merge( linearData.lineGeometry );
    const points = linearData.lineGeometry.vertices;
    const point = points[0];
    const particle = point.clone();
    particle.moveIndex = 0;
